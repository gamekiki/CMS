  ...
  <input type="hidden" name="gdProdSerno" value="<?=$gdProdSerno?>">
                     <table class="tblT01_detail" border="0" cellspacing="0" cellpadding="0">
                       ...
                       <tbody>             
                          <tr>
                            <th>상품 아이디 *</th>
                             <td><label><input name="prodId" maxlength="20" value="<?=$prodId?>" placeholder="" type="text"></label></td>
                          </tr>
                          <tr>
                            <th>상품 이미지 *</th>
                             <td>
                             <!-- <p class="p_txt">[권장사이즈] 가로 해상도 548px 이상</p> -->
                             <label><input name="prodImgUrl" maxlength="300" value="<?=$prodImgUrl?>" placeholder="이미지 파일 경로를 입력하세요." type="text"></label>
                             </td>
                          </tr>
                          <tr>
                            <th>상품명 *</th>
                             <td><label><input name="prodName" maxlength="10" value="<?=$prodName?>" placeholder="쇼핑몰에서 상품이미지 상단에 노출됩니다.(최대 10자)" type="text"></label></td>
                          </tr>
                          <tr>
                            <th>가격 *</th>
                             <td><label><input name="prodPrice" id="prodPrice" maxlength="7" value="<?=$prodPrice?>" onblur="chk_digit(this.value,'prodPrice','가격은')" placeholder="쇼핑몰에서 상품이미지 상단에 노출됩니다" type="text"></label></td>
                          </tr>
                          <tr>
                            <th>URL *</th>
                             <td><label><input name="prodUrl" maxlength="300" value="<?=$prodUrl?>" placeholder="" type="text"></label></td>
                          </tr>
                          <tr>
                            <th>키워드 *</th>
                             <td><label><input name="kword" maxlength="50" value="<?=$kword?>" placeholder="카테고리 명을 입력하세요" type="text"></label></td>
                          </tr>
                          <tr>
                            <th>수량 *</th>
                             <td><label><input name="prodQty" id="prodQty" maxlength="5" value="<?=$prodQty?>" onblur="chk_digit(this.value,'prodQty','수량은')" placeholder="수량을 입력하세요" type="text"></label></td>
                          </tr>
                         </tbody>
                      </table>
</form>
...
function go_write(){ // 미니 게임 등록
    ...
    document.frm.action = "./kiki/c112_product_modify.php";
	document.frm.submit();
}

function chk_digit(val, name, txt) { // 숫자만
	var patten = /^[0-9]*$/;
