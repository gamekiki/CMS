...
<?	$pre_page = $cur_page2 - 1;
	$next_page = $cur_page2 + 1;
	$page_per_block = 10;
	$member_page = 0;
	$block = 1;
	...
	//' 현재 페이지의 블럭 위치를 구한다.
	if ($cur_page2 % $page_per_block == 0 ) {
		$block = (int)($cur_page2 / $page_per_block);
	} else {
		$block = (int)($cur_page2 / $page_per_block) + 1;
	}
...
/*	if ($block > 1) {
	...
	IF ($pre_page >= 1) {
		echo ("<a href='Javascript:kiki_list($pre_page)'><img src='./kiki/img/icon_paging_prev.gif' alt='이전으로'></a>&nbsp; ");
	}	?>
<span class='page_num'>
<?	$direct_page = $first_page;
	  while($direct_page <= $last_page) {
		if ((int)($direct_page) == (int)($cur_page2)) {
