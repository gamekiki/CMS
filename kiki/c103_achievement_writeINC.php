...
$(document).ready(function(){
	...
	$('.kikimodal_pop .close').click(function (e) {
	  ...
	  $("#kiki_actId").val('');
	  $("#kiki_actiname").text('');
//	  $('input:hidden[name="rewardBadge"]').val('');
	});
...
<?	include "./kiki/kiki_header.php";
	$cur_page = kiki_ischar($_POST["cur_page"]);	?>
               ...
               <div class="kiki_row"  style="margin-top:50px">
                 <p class="tit">업적 생성</p>
                 <div class="con"> 
<form action="./kiki/c103_achievement_write.php" method="POST" name="kiki_frm">
                   <div class="kiki_tbview">
                     ...
                     <table class="tblT01_detail" cellspacing="0" cellpadding="0" border="0">
                       ...
                       <tbody>             
                          <tr>
                            <th>업적 이름</th>
                             <td><label><input name="achiName" value="" maxlength="15" placeholder="" type="text"></label></td>
                          </tr>
                          <tr>
                            <th>액티비티 선택</th>
                             <td><a href="#" onclick="wrapWindowByMask('#kiki_storage')" class="kikibtn white" style="padding:8px 10px; font-size:11px">액티비티 선택</a> &nbsp; <span id="kiki_actiname"></span></td>
                          </tr>
                          <tr>
                            <th>횟수</th>
                             <td>
                             <input name="kiki_count" id="kiki_count" value="" maxlength="2" value="" placeholder="" onblur="kiki_chk_digit(this.value,'kiki_count')" type="text" style="width:30%"> &nbsp; 회 </label>
                             </td>
                          </tr>
                       </tbody>
                      </table>
                     ...
                     <table class="tblT01_detail" cellspacing="0" cellpadding="0" border="0">
                       ...
                       <tbody>           
                          <tr class="kikipoint">
                            <th>포인트</th>
                             <td><label><input name="kiki_point" id="kiki_point" onblur="kiki_chk_digit(this.value,'kiki_point')" value="" maxlength="2" placeholder="" style=" width:70%" type="text"></label>&nbsp; point</td>
                          </tr>
                          <tr class="kikibadge">
                            <th>뱃지 </th>
                             <td>
...
<?		} // row while
	...
	<input type="hidden" name="appId" value="<?=$kiki_appId?>">
                   ...
                   <div class="kikibtnarea text_right">
                     <a href="javascript:history.back();" class="kikibtn"> 취 소 </a> <a href="javascript:kiki_checkForm();" class="kikibtn"> 업적 생성 </a>
                   </div>  
                   <!-- // btnarea -->             
...
//console.log("point="+ kiki_point)
//console.log("point="+ rewardBadge)
...
<?	} else {
		...
		if( $result_Q === false) {
			...
			while($row = mysqli_fetch_array($result_Q, MYSQLI_ASSOC)) {
			   ...
			   $rewardValue = stripslashes($rewardValue);	?>                           </tr>
                           
                           <tr>
                            <td><?=$actName?></td>
                            <td><? if($rewardValue) { ?> <?=$rewardValue?> pts<? } ?></td>
                            <td><a id="acti_btn_<?=$actId?>" href="javascript:select_activity('<?=$actId?>','<?=$actName?>');" class="kikibtn white" style="padding: 8px 10px 8px 10px; font-size:11px"> 선택 </a></td>
                           </tr>
<?	  } // row while
